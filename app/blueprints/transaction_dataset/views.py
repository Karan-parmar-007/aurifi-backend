...
def delete_sub_version():
    ...
    try:
        ...
        if delete_children:
            # Use the same logic as delete_rule_version
            return delete_rule_version()
        else:
            # Only delete this specific version
            # First, update children to point to this version's parent
            parent_id = version.get("parent_version_id")
            
            if parent_id:
                # Update all children to point to grandparent
                db["transaction_versions"].update_many(
            ...
            if success:
                return jsonify({
                    "status": "success",
                    "message": "Sub-version deleted successfully"
                }), 200
            else:
